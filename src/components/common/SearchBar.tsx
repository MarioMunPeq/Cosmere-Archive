      ...
      <div className="flex items-center gap-2 rounded-lg border border-gray-700/60 bg-gray-900/60 px-3 py-1.5 text-sm text-gray-300 transition-all focus-within:border-purple-500/60 focus-within:bg-gray-900">
        ...
        <input
          ...
          aria-activedescendant={
            highlightIdx >= 0 && results[highlightIdx]?.type !== 'header' ? `search-opt-${highlightIdx}` : undefined
          }
              ...
              results.map((r, i) => {
                if (r.type === 'header') {
                  return (
                    <div
                      key={r.label}
                      className="px-4 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-gray-600"
                    >
                      {r.label}
                    </div>
                  )
